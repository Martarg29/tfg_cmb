...
class UNet5(nn.Module):
    def __init__(self, channels,channel_in=1,channel_out=1,kernels_size=None,strides=None,
                 ...
                 mainBN=True,finalBN=True,eps=1e-05,momentum=0.1,sides=64):
        super(UNet5, self).__init__()
        if kernels_size is None:
            kernels_size = [3 for i in range(10)]
        if strides is None:
            strides = [1 for i in range(10)]
        if extra_pads is None:
            extra_pads = [0 for i in range(10)]
        c1 = seq.Conv2dSeq([channel_in, channels[0]],kernels_size=[kernels_size[0]],strides=[strides[0]],extra_pads=[extra_pads[0]],finalActive=mainActive,finalBN=mainBN,eps=eps,momentum=momentum,in_side=sides)
        self.conv1 = c1.get_seq()
        c2 = seq.Conv2dSeq([channels[0], channels[1]],kernels_size=[kernels_size[1]],strides=[strides[1]],extra_pads=[extra_pads[1]],finalActive=mainActive,finalBN=mainBN,eps=eps,momentum=momentum,in_side=c1.sides[-1])
        self.conv2 = c2.get_seq()
        c3 = seq.Conv2dSeq([channels[1], channels[2]],kernels_size=[kernels_size[2]],strides=[strides[2]],extra_pads=[extra_pads[2]],finalActive=mainActive,finalBN=mainBN,eps=eps,momentum=momentum,in_side=c2.sides[-1])
        self.conv3 = c3.get_seq()
        c4 = seq.Conv2dSeq([channels[2], channels[3]],kernels_size=[kernels_size[3]],strides=[strides[3]],extra_pads=[extra_pads[3]],finalActive=mainActive,finalBN=mainBN,eps=eps,momentum=momentum,in_side=c3.sides[-1])
        self.conv4 = c4.get_seq()
        c5 = seq.Conv2dSeq([channels[3], channels[4]],kernels_size=[kernels_size[4]],strides=[strides[4]],extra_pads=[extra_pads[4]],finalActive=mainActive,finalBN=mainBN,eps=eps,momentum=momentum,in_side=c4.sides[-1])
        self.conv5 = c5.get_seq()
        
        d1 = seq.Conv2dSeq([channels[4], channels[3]],kernels_size=[kernels_size[5]],strides=[strides[5]],extra_pads=[extra_pads[5]],finalActive=mainActive,finalBN=mainBN,eps=eps,momentum=momentum,transConv2d=True,in_side=c5.sides[-1])
        self.deconv1 = d1.get_seq()
        d2 = seq.Conv2dSeq([channels[3]*2, channels[2]],kernels_size=[kernels_size[6]],strides=[strides[6]],extra_pads=[extra_pads[6]],finalActive=mainActive,finalBN=mainBN,eps=eps,momentum=momentum,transConv2d=True,in_side=d1.sides[-1])
        self.deconv2 = d2.get_seq()
        d3 = seq.Conv2dSeq([channels[2]*2, channels[1]],kernels_size=[kernels_size[7]],strides=[strides[7]],extra_pads=[extra_pads[7]],finalActive=mainActive,finalBN=mainBN,eps=eps,momentum=momentum,transConv2d=True,in_side=d2.sides[-1])
        self.deconv3 = d3.get_seq()
        d4 = seq.Conv2dSeq([channels[1]*2, channels[0]],kernels_size=[kernels_size[8]],strides=[strides[8]],extra_pads=[extra_pads[8]],finalActive=mainActive,finalBN=mainBN,eps=eps,momentum=momentum,transConv2d=True,in_side=d3.sides[-1])
        self.deconv4 = d4.get_seq()
        d5 = seq.Conv2dSeq([channels[0]*2, channel_out], kernels_size=[kernels_size[9]], strides=[strides[9]], extra_pads=[extra_pads[9]], finalActive=finalActive, finalBN=finalBN, eps=eps, momentum=momentum, transConv2d=True, in_side=d4.sides[-1])
        self.deconv5 = d5.get_seq()
